...
export interface NavItemProps {
  ...
  icon?: ElementType;
  iconColor?: string;
  /** Custom leading slot — overrides icon. */
  leading?: ReactNode;
  count?: number;
  ...
  drillable?: boolean;
  /** Indent — used for nested folders. Defaults to 0. */
  indent?: 0 | 1;
  onClick?: () => void;
}
  ...
  return (
    <li>
      <Button
        onPress={onClick}
        className={cn(
          "w-full flex items-center gap-3 min-h-[42px] py-1.5 pr-3 select-none text-left outline-none cursor-pointer",
      ...
      >
        ...
        {drillable ? (
          <Icon name="chevron.right" size={13} className="text-fg-tertiary stroke-[2]" />
        ) : null}
      </Button>
    </li>
  );
