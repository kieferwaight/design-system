...
/**
 * The full Content Navigation View. Composes NavCard + NavItem from a single
 * NavCounts payload — which is exactly the shape the future `nav.counts`
 * WebSocket topic will deliver. Swapping mock → live data is a one-line change
      ...
      >
        {counts.ai_models.map((m) => (
          <NavItem
            key={m.slug}
            label={m.name}
            leading={<Avatar name={m.name} size="sm" square bg={m.accent} />}
            count={m.count}
